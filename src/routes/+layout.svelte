...
<nav class="nav">
	...
	<div class="nav-right">
		<a href="/dogs" class="nav-link" aria-label="Breeds"><span class="nav-emoji">🐾</span><span class="nav-text">{isZh ? '图鉴' : 'Breeds'}</span></a>
		<a href="/game" class="nav-link" aria-label="Game"><span class="nav-emoji">🏚️</span><span class="nav-text">{isZh ? '游戏' : 'Game'}</span></a>
		<a href="/workshop" class="nav-link" aria-label="Capsule"><span class="nav-emoji">💊</span><span class="nav-text">{isZh ? '胶囊' : 'Capsule'}</span></a>
		<a href="/kennels" class="nav-link" aria-label="Kennel">
			<span class="nav-emoji">🏠</span><span class="nav-text">{isZh ? '狗窝' : 'Kennel'}</span>
			{#if myKennels.length > 0}<span class="kennel-badge">{myKennels.length}</span>{/if}
		</a>
		<a href="https://github.com/PunkGo/punkgo-roast-web" target="_blank" rel="noopener" class="nav-link" aria-label="GitHub">
...
<style>
	...
	@media (max-width: 768px) {
		...
		.logo-title { font-size: 9px; letter-spacing: 0.2em; }
