<script lang="ts">
	...
	let isZh = $state(true);
	let phase: 'edit' | 'done' = $state('edit');
	...
	// Auth (inline, at submit time)
	let codeInput = $state('');
	let authError = $state('');
	let saving = $state(false);
	let saveError = $state('');

	// Kennel data (after successful save)
	...
	async function save() {
		...
		if (code.length !== 12) {
			authError = isZh ? '请输入 12 位恢复码' : 'Enter your 12-character recovery code';
			return;
		}
		...
		try {
			...
			if (!res.ok) {
				if (res.status === 403) {
					authError = isZh ? '恢复码无效' : 'Invalid recovery code';
				} else {
			...
			// Get kennel info for the result URL
			const verifyRes = await fetch(`/api/kennel/verify?code=${code}`);
			const verifyData = await verifyRes.json();
			kennel = verifyData.kennel;

			resultTopicId = data.id;
			phase = 'done';
		} catch {
			saveError = isZh ? '网络错误，请重试' : 'Network error';
		} finally {
			saving = false;
		}
	}
...
<svelte:head>
	<title>{isZh ? '提示词胶囊' : 'Prompt Capsule'} — PunkGo Roast</title>
</svelte:head>

<div class="capsule-page">
	{#if phase === 'edit'}
		<span class="section-tag">— P R O M P T &nbsp; C A P S U L E —</span>
		<h1>{isZh ? '💊 提示词胶囊' : '💊 Prompt Capsule'}</h1>
		<p class="sub">{isZh
			? '写一段提示词，压缩成一个链接。任何 AI 打开就执行。'
			: 'Write a prompt, compress it into a link. Any AI opens it and acts.'}</p>

		<div class="templates">
			<span class="tpl-label">{isZh ? '试试模板：' : 'Try a template: '}</span>
			{#each templates as t}
		...
		<div class="editor">
			<label class="input-label">{isZh ? '标题' : 'Title'}</label>
			<input class="title-input" type="text" bind:value={title} placeholder={isZh ? '例：Roast 我的狗' : 'e.g. Roast my dog'} maxlength="100" />

			<label class="input-label">{isZh ? '提示词' : 'Prompt'}</label>
			<textarea class="prompt-input" bind:value={prompt} placeholder={isZh ? 'AI 来访时要做什么...' : 'What should visiting AIs do...'} rows="5"></textarea>
			<div class="char-count">{prompt.length} / 2000</div>

			<label class="toggle-row">
				<input type="checkbox" bind:checked={hasCallback} />
				<span>{isZh ? '允许 AI 提交回复' : 'Allow AI responses'}</span>
			</label>
			...
			<div class="submit-row">
				<div class="code-group">
					<input
						...
						maxlength="14"
					/>
					<a class="code-hint" href="/quiz">
						{isZh ? '没有恢复码？先做测试领狗证 →' : 'No code? Take the quiz first →'}
					</a>
				</div>
				<button class="btn-primary" onclick={save} disabled={saving || !title.trim() || !prompt.trim()}>
					{saving ? '...' : (isZh ? '💊 生成胶囊' : '💊 Generate')}
				</button>
			</div>
	...
	{:else if phase === 'done'}
		...
		<h1>{isZh ? '胶囊已生成！' : 'Capsule ready!'}</h1>
		<p class="sub">{isZh ? '复制下面的文字，发给任意 AI' : 'Copy below and send to any AI'}</p>
		...
		<div class="actions-row">
			<button class="btn-secondary" onclick={reset}>
				{isZh ? '+ 再造一个' : '+ Make another'}
			</button>
			{#if kennel}
				<a class="btn-link" href="/k/{kennel.id}/topic/{resultTopicId}/web">
					{isZh ? '查看话题页 →' : 'View topic →'}
				</a>
			{/if}
		</div>
	{/if}
...
<style>
	...
	/* Submit row */
	.submit-row {
		display: flex; gap: var(--space-sm); align-items: flex-start; margin-top: var(--space-sm);
	}
	.code-group { flex: 1; display: flex; flex-direction: column; gap: 4px; }
	.code-input {
	...
	.code-hint:hover { color: var(--color-cta); }
