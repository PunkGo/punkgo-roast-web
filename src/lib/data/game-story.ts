...
"Blackwell 先生带着仪器和工具独自在二楼待了三个夜晚。他出来时，走廊似乎变短了，虽然我说不上为什么。"`,
	...
	// Round 3 — The Measurement Mystery
	{
		...
		narrativeEn: `You take precise measurements with a laser rangefinder. The hallway from the Library entrance to the Conservatory entrance: 8.2 meters.
...
The missing room is real. It's hidden inside the walls.`,
		narrativeZh: `你用激光测距仪做了精确测量。从图书馆入口到温室入口的走廊长度：8.2 米。
...
消失的房间是真实的。它藏在墙壁里面。`,
	...
	// Round 4 — The Architect's Signature
	{
		...
		imageAltEn: 'Close-up of a 7-petal flower in the wallpaper; a brass compass plate with E circled',
		imageAltZh: '壁纸中七瓣花特写；E 被圈出的黄铜罗盘盘',
...
Second, in the Library, behind a row of encyclopedias on the deep shelf's back panel: a small brass plate, roughly 5cm across. It's engraved with a compass rose — N, S, E, W — but the letter E is circled.
...
第二，图书馆里，深书架背板后面一排百科全书后面：一块小黄铜片，大约 5 厘米。上面刻着罗盘玫瑰—— N、S、E、W——但字母 E 被画了圈。

"E" 代表东方？还是 "E" 代表 Edmund？`,
		clueEn: `Two hidden markers found: (1) A 7-petal flower anomaly on the hallway wallpaper at eye level. (2) A brass compass rose plate behind Library bookshelves with "E" circled. The architect left deliberate markers. "E" could mean East, Edmund (the architect), or both.`,
		clueZh: `发现两个隐藏标记：(1) 走廊壁纸上视线高度处有一朵 7 瓣花异常。(2) 图书馆书架后有黄铜罗盘盘，"E" 被圈出。建筑师留下了刻意的标记。"E" 可能代表东方、Edmund（建筑师），或两者皆是。`,
		choices: [
		...
		summaryEn: (c) => `Found two markers: a 7-petal flower in hallway wallpaper, and a brass compass plate with "E" circled in the Library. Chose to ${c === 'A' ? 'press the flower' : c === 'B' ? 'try the compass plate' : 'search for more symbols'}.`,
		summaryZh: (c) => `发现两个标记：走廊壁纸上的七瓣花，图书馆里"E"被圈出的黄铜罗盘。选择${c === 'A' ? '按下花朵' : c === 'B' ? '尝试罗盘盘' : '寻找更多符号'}。`,
		floorPlan,
	},
	...
	// Round 5 — The Turning Point
	{
		...
		narrativeEn: `You try both mechanisms. Pressing the 7-petal flower — you hear a faint click from inside the wall, but nothing visible happens. Rotating the compass plate so the circled "E" points downward — a mechanical grinding sound, deep within the structure. But still, nothing opens.
...
Edmund Blackwell didn't just hide a room. He built mechanized walls that can slide. Two triggers activated, but the system needs a third input to complete the circuit. The architect was known to use the number three in his other buildings — three chimneys, three-step staircases, triangular garden layouts.`,
		narrativeZh: `你尝试了两个机关。按下七瓣花——墙内传来一声轻微的咔嗒声，但什么也没发生。转动罗盘盘让圈出的"E"朝下——深处传来机械研磨声。但墙壁仍然没有打开。
